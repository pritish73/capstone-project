"""
Debug script for Uncertainty-Guided Transfer Learning
"""
import numpy as np
import sys
import os
...
def debug_ugtl(data, window_len=10):
    """
    Debug version of UGTL with prints
    """
    ...
    for test_idx, test_id in enumerate(unique_ids[:5]):  # Only first 5 subjects for debug
        ...
        if not subject_models:
            ...
            pred_mean, _ = overall_model.predict(features_test_std, return_std=True)
        ...
        for i in range(len(features_test_std)):
            test_sample = features_test_std[i:i+1]
            ...
            for train_id, model_info in subject_models.items():
                ...
                subj_mean, subj_std = subject_model.predict(test_sample, return_std=True)
                ...
                if len(features_subject_train) > 0:
                    mean_subj = model_info['mean']
                    std_subj = model_info['std']
                    features_subject_train_std = (features_subject_train - mean_subj) / std_subj

                    distances = np.sqrt(np.sum((features_subject_train_std - test_sample)**2, axis=1))
                ...
                reliability = 1.0 / (subj_std + 1e-8)
                combined_weight = similarity * reliability

                subject_weights.append(combined_weight)
                subject_preds.append(subj_mean[0])
                subject_stds.append(subj_std[0])
...
def main():
    ...
    print("\n=== Debugging Uncertainty-Guided Transfer Learning ===")
    pred, true = debug_ugtl(data, window_len=10)
