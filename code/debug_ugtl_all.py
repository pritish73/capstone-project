"""
Debug script for Uncertainty-Guided Transfer Learning on all subjects
"""
import numpy as np
import sys
import os
...
def debug_ugtl_all(data, window_len=10):
    """
    UGTL on all subjects, returns predictions, targets, uncertainties
    """
    ...
    if not all_features:
        print("No subjects with sufficient data")
    ...
    all_pred = []
    all_true = []
    all_unc = []  # uncertainty estimates

    for test_idx, test_id in enumerate(unique_ids):
        print(f"\nProcessing test subject {test_id} ({test_idx+1}/{n_subjects})")
        ...
        all_true.append(targets_test)
        all_unc.append(np.array(test_uncertainties))
    ...
    unc_vals = np.concatenate(all_unc)

    print(f"\nFinal predictions shape: {pred_mean.shape}")
    print(f"Final true values shape: {true_vals.shape}")
    print(f"Final uncertainties shape: {unc_vals.shape}")

    r2 = r2_score(true_vals, pred_mean)
    print(f"Final R²: {r2:.4f}")

    # Compute Expected Calibration Error (ECE) and coverage
    # For regression, we can compute prediction intervals using uncertainty
    # Assuming Gaussian predictive distribution, 90% interval: mean +/- 1.645 * std
    alpha = 0.1  # for 90% interval
    z = np.abs(np.random.standard_normal(1))  # placeholder, actually use norm.ppf
    from scipy import stats
    z_score = stats.norm.ppf(1 - alpha/2)  # two-sided
    lower = pred_mean - z_score * unc_vals
    upper = pred_mean + z_score * unc_vals
    coverage = np.mean((true_vals >= lower) & (true_vals <= upper))
    print(f"Coverage ({1-alpha*100:.0f}% interval): {coverage:.4f}")

    # Compute average uncertainty
    avg_unc = np.mean(unc_vals)
    print(f"Average uncertainty: {avg_unc:.4f}")

    # Compute Empirical CDF error? We'll compute ECE as average absolute difference
    # between empirical coverage and expected confidence.
    # We'll compute for multiple confidence levels.
    conf_levels = np.arange(0.1, 1.0, 0.1)
    ece_total = 0.0
    for conf in conf_levels:
        z = stats.norm.ppf(1 - (1-conf)/2)
        lower_c = pred_mean - z * unc_vals
        upper_c = pred_mean + z * unc_vals
        empirical = np.mean((true_vals >= lower_c) & (true_vals <= upper_c))
        ece_total += abs(empirical - conf)
    ece = ece_total / len(conf_levels)
    print(f"Expected Calibration Error (ECE): {ece:.4f}")

    return pred_mean, true_vals, unc_vals
...
def main():
    ...
    print("\n=== Uncertainty-Guided Transfer Learning (All Subjects) ===")
    pred, true, unc = debug_ugtl_all(data, window_len=10)
