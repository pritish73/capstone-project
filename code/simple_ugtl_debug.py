"""
Simple UGTL debug - based on what worked in debug_ugtl_fixed.py
"""
import numpy as np
import sys
import os
...
def simple_ugtl(data, window_len=10):
    """
    Simple UGTL based on debug_ugtl_fixed.py that worked
    """
    ...
    for test_idx, test_id in enumerate(unique_ids):
        if test_idx % 5 == 0:
            print(f"Processing test subject {test_id} ({test_idx+1}/{n_subjects})")
...
def main():
    ...
    print("\n=== Simple UGTL Debug ===")
    pred, true, unc = simple_ugtl(data, window_len=10)
